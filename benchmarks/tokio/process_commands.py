...
for i in range(numCommands):
    ...
    for lib in libs:
        if lib.endswith(".o") and lib not in command_to_objectFiles[command]:
...
for command in commands:
    ...
    finalBinary = finalBinary.replace(currDir+"/",'')
    ...
    for obj in command_to_objectFiles[command]:
        ...
        svf_command = SVF + " "+stripped_obj+ " "+SVFArgs
        svf_command = shlex.split(svf_command)
        execute_command(svf_command,"Completed Pointer Analysis successfully", "SVF failed to complete")
    ...
    compile_command = llvmCompiler + " "+finalLinkable + llvmCompilerFlags
