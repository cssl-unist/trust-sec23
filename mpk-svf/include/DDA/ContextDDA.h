 ...
 */
    ...
    public:
        ...
        virtual const CxtPtSet& computeDDAPts(const CxtVar& cxtVar, bool clearAll);

        /// Handle out-of-budget dpm
        void handleOutOfBudgetDpm(const CxtLocDPItem& dpm);

        const LocToDPMVecMap getLocToDPMMap(NodeID id){
            ContextCond cxt;
            CxtVar var(cxt, id);
            computeDDAPts(var, true);
            LocToDPMVecMap tt = getLocToDPMVecMap();
            id = getDefSVFGNode(getPAG()->getPAGNode(id))->getId();
            //assert(tt.find(id) != tt.end());
            return tt;
        }

        const set<CxtLocDPItem> getFinalStackDpms(){
            return getFinalStackDpmSet();
        }

        const set<CxtLocDPItem> getFinalHeapDpms(){
            return getFinalHeapDpmSet();
        }
        ...
        virtual void addCallString(const CxtLocDPItem& dpm) override;
