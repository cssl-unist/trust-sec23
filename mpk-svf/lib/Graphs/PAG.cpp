    ...
    GenericPAGNodeTy(i,k), value(val)
{
    ...
    case GepValNode:
    {
        ...
        isTLPointer = val->getType()->isPointerTy() || val->getType()->isAggregateType() && SymbolTableInfo::SymbolInfo()->enclosesPointer(val->getType());
        isATPointer = false;
        break;
    }
    ...
    case RetNode:
    {
        ...
        isTLPointer = SVFUtil::cast<Function>(val)->getReturnType()->isPointerTy()
                      || (SVFUtil::cast<Function>(val)->getReturnType()->isAggregateType()
                      && SymbolTableInfo::SymbolInfo()->enclosesPointer(SVFUtil::cast<Function>(val)->getReturnType()));
        isATPointer = false;
        break;
    }
