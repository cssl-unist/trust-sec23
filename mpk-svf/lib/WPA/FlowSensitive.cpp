...
bool FlowSensitive::processSVFGNode(SVFGNode* node)
{
    ...
    else if(SVFUtil::isa<CmpVFGNode>(node) || SVFUtil::isa<BinaryOPVFGNode>(node) || SVFUtil::dyn_cast<UnaryOPVFGNode>(node))
    {
        ...
        if(processInsertValue(ivNode)){
            changed = true;
        }
    }else if(ExtractValVFGNode* evNode = SVFUtil::dyn_cast<ExtractValVFGNode>(node)){
        if(processExtractValue(evNode)){
            changed = true;
        }
    }
    else
...
bool FlowSensitive::processInsertValue(const InsertValVFGNode *insertVal) {

    double start = stat->getClk();
    bool changed = false;
    const PointsTo& srcPts = getPts(insertVal->getInserted()->getId());
    NodeID ivID = insertVal->getRes()->getId();
    const LocationSet& ls = insertVal->getLocationSet();
    PointsTo tmpDstPts;

    for (NodeID o : srcPts)
    {
        pag->addInsertValObjNode(ivID,ls,o);
        tmpDstPts.set(o);
    }

    if(!pag->isNullPtr(insertVal->getParent()->getId())){
        set<pair<const LocationSet, NodeID>> pts = pag->getInsertValObjNodes(insertVal->getParent()->getId());
        for(auto p: pts){
            pag->addInsertValObjNode(ivID,p.first,p.second);
            tmpDstPts.set(p.second);
        }
    }

    if (unionPts(insertVal->getRes()->getId(), tmpDstPts))
        changed = true;
...
bool FlowSensitive::processExtractValue(const ExtractValVFGNode *extractVal) {
    double start = stat->getClk();
    bool changed = false;
    const PointsTo& srcPts = getPts(extractVal->getPAGSrcNodeID());
    const LocationSet ls = SVFUtil::cast<ExtractValPE>(extractVal->getPAGEdge())->getLocationSet();

    PointsTo tmpDstPts;

    for (NodeID o : srcPts)
    {
        if(pag->getExtractValNodes(o,ls)){
            tmpDstPts.set(o);
        }
    }

    if (unionPts(extractVal->getPAGDstNodeID(), tmpDstPts))
        changed = true;
