...
fn make_mir_scope(
    ...
    let is_unsafe = match scope_data.is_unsafe {
        true => true,
        _ => false
    };
    ...
    let dbg_scope = match scope_data.inlined {
        ...
        None => unsafe {
            llvm::LLVMRustDIBuilderCreateLexicalBlock(
                ...
                //if scope_data.is_unsafe {1} else {0},
            )
            //
            // if scope_data.local_data.as_ref().assert_crate_local().safety != Safety::Safe {
            //     llvm::LLVMRustDIBuilderSetUnsafe(DIB(cx),ll_block);
            // }
            // ll_block
        },
    };
    ...
    let inlined_at = scope_data.inlined.map(|(_, callsite_span)| {
        ...
        //cx.dbg_loc(callsite_scope, parent_scope.inlined_at, callsite_span, if scope_data.is_unsafe {true} else {false})
        cx.dbg_loc(callsite_scope, parent_scope.inlined_at, callsite_span, true)
    });
