        ...
        + HasParamEnv<'tcx>,
{
    ...
    fn pointee_info_at(this: TyAndLayout<'tcx>, cx: &C, offset: Size) -> Option<PointeeInfo> {
        ...
        //MPK-Immut
        let is_ptr_immut = !this.ty.is_mutable_ptr();
        let is_ptr_unsafe = this.ty.is_unsafe_ptr();
        let pointee_info = match *this.ty.kind() {
            ty::RawPtr(mt) if offset.bytes() == 0 => {
                cx.layout_of(mt.ty).to_result().ok().map(|layout| PointeeInfo {
                    ...
                    ptr_immut: is_ptr_immut,
                    ptr_unsafe: is_ptr_unsafe
                })
            }
    ...
    fn new_internal(
        ...
        let adjust_for_rust_scalar = |attrs: &mut ArgAttributes,
            ...
            if let Some(pointee) = layout.pointee_info_at(cx, offset) {
                if  pointee.ptr_immut {
                    attrs.set(ArgAttribute::MPKImmut);
                }

                if  pointee.ptr_unsafe && sig.unsafety == hir::Unsafety::Unsafe {
                    attrs.set(ArgAttribute::MPKUnsafe);
                }
