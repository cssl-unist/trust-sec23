...
//MPK-Extern Attribute
static void handleMPKExternAttr(Sema &S, Decl *D, const AttributeList &Attr){
  if(!isFunctionOrMethod(D)){
    S.Diag(D->getLocation(), diag::warn_attribute_wrong_decl_type)
         << "'mpk_extern'" << ExpectedFunctionOrMethod;
      return;
  }

  handleSimpleAttribute<MPKExternAttr>(S, D, Attr);
}
