...
static Attribute::AttrKind getAttrFromCode(uint64_t Code) {
  ...
  //MPK-extern
  case bitc::ATTR_KIND_MPK_EXTERN:
    return Attribute::MPKExtern;
  case bitc:: ATTR_KIND_MPK_IMMUT:
    return Attribute::MPKImmut;
  case bitc::ATTR_KIND_MPK_UNSAFE:
    return Attribute::MPKUnsafe;
...
Error BitcodeReader::parseFunctionBody(Function *F) {
  ...
  while (true) {
    ...
    case bitc::FUNC_CODE_DEBUG_LOC: {      // DEBUG_LOC: [line, col, scope, ia, unsafe]
      I = getLastInstruction();
      if (!I || Record.size() < 5)
        return error("Invalid record");
      ...
      bool isInUnsafeBlock = Record[4]; //MPK-Unsafe
      bool isImplicitCode = Record.size() == 6 && Record[5];
      ...
      LastLoc = DebugLoc::get(Line, Col, Scope, IA, isInUnsafeBlock, isImplicitCode);
      I->setDebugLoc(LastLoc);
      I = nullptr;
      continue;
    }
