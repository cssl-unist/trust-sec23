...
#define GET_OR_DISTINCT(CLASS, ARGS)                                           \
  ...
  case bitc::METADATA_LOCATION: {
    if (Record.size() != 5 && Record.size() != 6 && /*MPK-Unsafe-edit*/ Record.size() != 7)
      return error("Invalid record");
    ...
    bool InUnsafeBlock = Record[5]; //MPK-Unsafe
    bool ImplicitCode = Record.size() == 7 && Record[6];

    MetadataList.assignValue(
        GET_OR_DISTINCT(DILocation, (Context, Line, Column, Scope, InlinedAt,
                                     ImplicitCode, InUnsafeBlock)),
        NextMetadataNo);
    NextMetadataNo++;
    break;
  }
