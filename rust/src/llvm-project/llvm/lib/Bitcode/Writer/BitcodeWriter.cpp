...
static uint64_t getAttrKindEncoding(Attribute::AttrKind Kind) {
  switch (Kind) {
    //MPK-extern
  case Attribute::MPKExtern:
    return bitc::ATTR_KIND_MPK_EXTERN;
  case Attribute::MPKImmut:
    return bitc::ATTR_KIND_MPK_IMMUT;
  case Attribute::MPKUnsafe:
    return bitc::ATTR_KIND_MPK_UNSAFE;
...
void ModuleBitcodeWriter::writeDILocation(const DILocation *N,
  ...
  //MPK-Unsafe
  Record.push_back(N->isInUnsafeBlock());
...
void ModuleBitcodeWriter::writeFunction(
  ...
  for (Function::const_iterator BB = F.begin(), E = F.end(); BB != E; ++BB)
    for (BasicBlock::const_iterator I = BB->begin(), E = BB->end();
      ...
      //MPK-Unsafe
      Vals.push_back(DL->isInUnsafeBlock());
