...
//MPK-Unsafe
bool DebugLoc::isInUnsafeBlock() const {
    if(DILocation *Loc = get()){
        return Loc->isInUnsafeBlock();
    }
    return false;
}

void DebugLoc::setInUnsafeBlock(bool InUnsafeBlock){
    if(DILocation* Loc = get()){
        Loc->setInUnsafeBlock(InUnsafeBlock);
    }
}
...
DebugLoc DebugLoc::get(unsigned Line, unsigned Col, const MDNode *Scope,
                       const MDNode *InlinedAt, /*MPK-Unsafe*/bool InUnsafeBlock, bool ImplicitCode) {
  ...
  return DILocation::get(Scope->getContext(), Line, Col,
                         ...
                         const_cast<MDNode *>(InlinedAt), /*MPK-Unsafe*/InUnsafeBlock, ImplicitCode);
}
