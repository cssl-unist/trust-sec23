...
template <> struct MDNodeKeyImpl<DILocation> {
  ...
  Metadata *Scope;
  Metadata *InlinedAt;
  bool InUnsafeBlock; //MPK-Unsafe
  bool ImplicitCode;


  MDNodeKeyImpl(unsigned Line, unsigned Column, Metadata *Scope,
                Metadata *InlinedAt, /*MPK-Unsafe*/ bool InUnsafeBlock, bool ImplicitCode)
      : Line(Line), Column(Column), Scope(Scope), InlinedAt(InlinedAt), InUnsafeBlock(InUnsafeBlock),
        ImplicitCode(ImplicitCode) {}
  MDNodeKeyImpl(const DILocation *L)
      : Line(L->getLine()), Column(L->getColumn()), Scope(L->getRawScope()),
        InlinedAt(L->getRawInlinedAt()), /*MPK-Unsafe*/InUnsafeBlock(L->isInUnsafeBlock()), ImplicitCode(L->isImplicitCode()) {}

  bool isKeyOf(const DILocation *RHS) const {
    return Line == RHS->getLine() && Column == RHS->getColumn() &&
           Scope == RHS->getRawScope() && InlinedAt == RHS->getRawInlinedAt() &&
           /*MPK-Unsafe*/ InUnsafeBlock == RHS->isInUnsafeBlock() &&
           ImplicitCode == RHS->isImplicitCode();
  }

  unsigned getHashValue() const {
    return hash_combine(Line, Column, Scope, InlinedAt, /*MPK-Unsafe*/InUnsafeBlock, ImplicitCode);
  }
};
